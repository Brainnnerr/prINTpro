...
export default function OrderWizard({ service, onBack }: OrderWizardProps) {
  ...
  useEffect(() => {
    ...
    const sheetsUsed = config.sideMode === "double"
      ? Math.ceil(pages / 2)
      : pages;
 ...
 const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[40px] p-10 shadow-2xl border border-red-100 text-center animate-in zoom-in duration-300">
          ...
          <p className="text-sm text-slate-500 font-medium mt-3 leading-relaxed">
            Your PrintPro account has been disabled by the administrator due to
            violations or outstanding issues.
          </p>
          <button
            onClick={onBack}
  ...
  const isStep1Disabled = !config.customerName ||
    (config.fulfillment === "delivery" ? !config.address : !config.pickupTime);
      ...
      <div className="w-full max-w-xl bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div className="bg-slate-900 px-8 py-8 text-white flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-black uppercase tracking-tight leading-none mb-1">
                {service?.name}
              </h2>
        ...
        <div className="p-8">
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() =>
                    setConfig({ ...config, fulfillment: "pickup" })}
                ...
                >
                  <Store size={18} />{" "}
                  <span className="font-bold text-xs">Pick-up</span>
                </button>
                <button
                  type="button"
                  onClick={() =>
                    setConfig({ ...config, fulfillment: "delivery" })}
                  className={`p-4 rounded-2xl border-2 transition-all flex items-center gap-3 ${
                ...
                : (
                  <div className="relative">
                    ...
                    <input
                      ...
                      onBlur={() =>
                        !config.pickupTime && setTimeInputType("text")}
                      placeholder="Preferred Pickup Time"
                      title="Choose pickup time"
                      value={config.pickupTime}
                      onChange={(e) =>
                        setConfig({ ...config, pickupTime: e.target.value })}
                      className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-emerald-500 font-bold text-sm"
                    />
                  </div>
                )}
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in">
                  ...
                  >
                    {service?.id === "s3"
                      ? (
                        ...
                        <>
                          <option value="A4">
                            Standard A4 (8.27" x 11.69")
                          </option>
                          <option value="Letter">
                            Letter / Short (8.5" x 11")
                          </option>
                          <option value="Long">
                            Long / Folio (8.5" x 13")
                          </option>
                      ...
                      ? (
                        /* Rush ID Photos */
                        <>
                          <option value="2x2 (6pcs)">
                            2x2 Set (6pcs) - ₱25
                          </option>
                          <option value="1x1 (6pcs)">
                            1x1 Set (6pcs) - ₱25
                          </option>
                          <option value="Passport Size (6pcs)">
                            Passport Size (6pcs) - ₱25
                          </option>
                          <option value="Combo Set">
                            Combo (Mixed Sizes) - ₱25
                          </option>
                        </>
                      )
                      : service?.id === "s5"
                      ? (
                        /* Photo Printing */
                        <>
                          <option value="3R">3R Size (3.5" x 5") - ₱15</option>
                          <option value="4R">4R Size (4" x 6") - ₱15</option>
                          <option value="5R">5R Size (5" x 7") - ₱15</option>
                        </>
                      )
                      : (
                        /* Fallback */
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-100">
                    <button
                      type="button"
                      onClick={() =>
                        setConfig({ ...config, sideMode: "single" })}
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setConfig({ ...config, sideMode: "double" })}
                      className={`flex-1 py-3 rounded-xl text-[10px] font-black ${
              ...
              <textarea
                placeholder="Description / Additional Instructions"
...
/>
              <button
                ...
                className={`w-full border-2 border-dashed rounded-[32px] p-10 transition-all ${
                  config.file
                    ...
                    <>
                      <Upload
                        className={`mx-auto mb-2 ${
                          config.file ? "text-emerald-500" : "text-slate-300"
                        }`}
                        size={32}
                      />
                      <p className="text-xs font-bold text-slate-600 truncate px-4">
                        {config.file ? config.file.name : "Upload Assets"}
                      </p>
                      {config.pageCount > 0 && (
                        <span className="mt-2 inline-block text-[9px] font-black bg-emerald-500 text-white px-2 py-0.5 rounded shadow-sm">
                          {config.pageCount} PAGES DETECTED
                        </span>
                      )}
                    </>
                  )}
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-slate-50 rounded-[32px] p-6 border border-slate-100 space-y-4">
                <div className="flex justify-between text-xs font-bold">
                  ...
                  <span className="text-slate-900">{config.customerName}</span>
                </div>
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-slate-400">Variant</span>
                  <span className="text-slate-900">{config.paperSize}</span>
                </div>
                <div className="flex justify-between text-xs font-bold">
                ...
                <div className="pt-4 border-t border-slate-200 flex justify-between items-center">
                  ...
                  <span className="text-2xl font-black text-emerald-600">
                    ₱{config.calculatedPrice.toFixed(2)}
                  </span>
                </div>
              </div>
              <div className="flex gap-2 p-4 bg-blue-50 rounded-2xl text-blue-700">
                ...
                <p className="text-[10px] font-bold leading-tight uppercase">
                  Manual Payment: Pay personally to the admin upon arrival.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="animate-in zoom-in duration-500">
                ...
                <div className="space-y-2.5 text-[11px] font-bold uppercase text-slate-600 mb-6">
                  ...
                  {config.fulfillment === "pickup"
                    ? (
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-slate-900 font-black">
                          {config.pickupTime
                            ? formatTimeAMPM(config.pickupTime)
                            : "--:-- --"}
                        </span>
                      </div>
                    )
                    : (
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-slate-900 normal-case font-medium line-clamp-2">
                          {config.address}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-dashed border-slate-200 mt-2 text-sm font-black text-slate-900">
                    ...
                    <span className="text-emerald-600 text-lg font-black">
                      ₱{orderReceipt.total_price.toFixed(2)}
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-amber-50 p-4 rounded-2xl flex gap-3 text-left mb-6 border border-amber-100">
                ...
                <p className="text-[10px] font-bold text-amber-800 uppercase leading-tight tracking-wide">
                  Important: Take a screenshot of this receipt. The QR Code must
                  be clear for the admin to verify your requisition and manual
                  payment.
                </p>
              </div>
          ...
          {step < 4 && (
            <div className="mt-8 flex gap-3">
              <button
                ...
                onClick={prevStep}
                className="flex-1 py-4 border border-slate-200 rounded-2xl font-bold text-slate-400 hover:bg-slate-50 transition-all text-xs"
              >
                BACK
              </button>
              <button
                ...
                disabled={loading || (step === 1 && isStep1Disabled) ||
                  (step === 2 && !config.file)}
